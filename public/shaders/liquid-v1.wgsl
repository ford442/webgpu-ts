...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Read the depth from the PREVIOUS frame's texture.
    ...
    // The "dynamic_depth" is now the depth from the previous frame.
    let dynamic_depth = original_depth;
    ...
    let shadow_intensity = smoothstep(0.4, 0.9, dynamic_depth) * 0.85;
    color = mix(color, shadow_color, shadow_intensity);

    let foreground_fog_color = vec3<f32>(0.6, 0.6, 0.7);
    let foreground_fog_intensity = smoothstep(0.2, 0.8, 1.0 - dynamic_depth) * 0.18;
    ...
    // --- ADD THIS BLOCK ---
    // Advect the depth map for the next frame.
    // We sample the depth map from its "old" position (displacedUV) and write it
    // to the "new" position (global_id.xy) for the next frame to read.
    let displacedDepthValue = textureSampleLevel(readDepthTexture, non_filtering_sampler, displacedUV, 0.0).r;
    textureStore(writeDepthTexture, global_id.xy, vec4<f32>(displacedDepthValue, 0.0, 0.0, 0.0));
}
