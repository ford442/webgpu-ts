...
@fragment
fn fs_main(@builtin(frag_coord) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
  let resolution = vec2<f32>(textureDimensions(u_texture, 0));
  var uv = frag_coord.xy / resolution;

  // Sample the GUI image
  let base = textureSample(u_texture, u_sampler, uv).rgb;

  // Simple button mapping: we'll treat up to 6 buttons arranged as rectangles at hardcoded UV positions.
  // Button states are encoded in u.zw and the fractional part of u.x: we store a bitmask in u.z (for up to 32 buttons)
  // To make JS -> shader simple, we will write a float 'buttonMask' into u.z where bit i indicates button i.
  let time = u.x;
  let buttonMask = u.z;

  // Define 6 example button rects (x0,y0,x1,y1) in UV space
  let rects = array<vec4<f32>, 6>(
    vec4<f32>(0.05, 0.75, 0.20, 0.88),
    vec4<f32>(0.25, 0.75, 0.40, 0.88),
    vec4<f32>(0.45, 0.75, 0.60, 0.88),
    vec4<f32>(0.65, 0.75, 0.80, 0.88),
    vec4<f32>(0.05, 0.58, 0.20, 0.70),
    vec4<f32>(0.25, 0.58, 0.40, 0.70)
  );
  ...
  for (var i: i32 = 0; i < 6; i = i + 1) {
    ...
    if (uv.x >= r.x && uv.x <= r.z && uv.y >= r.y && uv.y <= r.w) {
      let bit = f32((i32(buttonMask) >> i) & 1);
      if (bit > 0.5) {
