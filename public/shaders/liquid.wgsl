...
fn get_displacement(uv: vec2<f32>) -> DisplacementResult {
    ...
    // Ambient wave
    total_offset += vec2<f32>(sin(uv.x * 15.0 + currentTime * 0.5) * 0.02, cos(uv.y * 15.0 * 0.7 + currentTime * 0.5) * 0.02);
    ...
    for (var i: u32 = 0u; i < rippleCount; i = i + 1u) {
        ...
        if (timeSinceClick > 0.0 && timeSinceClick < 3.0) {
            ...
            if (dist > 0.0001) {
                ...
                // Add to the strength, clamped for stability
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let blur_radius = displacement.ripple_strength * pixel * 2.0; // Blur more when agitated
    ...
    // Mix between sharp and blurred based on ripple strength
